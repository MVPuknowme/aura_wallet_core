...
A GitHub Actions workflow sends a weekly status email every Monday at 12:00 UTC (and can also be triggered manually for dry-run validation). Configure the following repository secrets so the workflow can authenticate with your SMTP provider:
...
You can customize the email subject, body, or schedule in `.github/workflows/weekly-email.yml`. Manual `workflow_dispatch` supports `dry_run` (`true`/`false`) so you can validate logs without sending mail. The workflow uses a `concurrency` group to ensure only one weekly email run is active at a time.
